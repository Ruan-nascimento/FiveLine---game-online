-- Timeout of a turn skips to the opponent instead of ending the game.
...
begin
  ...
  if v_result is null and v_game.move_count + 1 = 225 then
    v_result := 'draw';
    v_reason := 'board_full';
  end if;

  insert into public.game_moves (game_id, player_id, move_number, row, col, piece)
  values (p_game_id, v_user, v_game.move_count + 1, p_row, p_col, v_piece);
...
grant execute on function public.forfeit_timed_out_turn(uuid), public.submit_game_move(uuid, smallint, smallint) to authenticated;
